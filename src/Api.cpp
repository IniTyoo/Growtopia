...
int Api::L_FINDPATH(lua_State* l) {
	...
	delay = delay < 0 ? 0 : delay > 2000 ? 2000 : delay;
	ENetClient* client = L_GC(l);
	if (client)
		lua_pushboolean(l, client->FindPath(luaL_checkinteger(l, 1), luaL_checkinteger(l, 2), delay));
	return 1;
}
...
void Api::OPEN(lua_State* l, uintptr_t addr) {
	...
	lua_register(l, "LogToConsole", L_LOGTOCONSOLE);
	...
	lua_register(l, "GetObjectList", L_GETOBJECTLIST);
