...
bool ClientData::Flush() {
	try {
		...
		http::Response response = request.send("POST", "version=3%2E95&platform=4&protocol=" + std::to_string(protocol), {"Content-Type: application/x-www-form-urlencoded"});
		std::string content{response.body.begin(), response.body.end()};
		
		for (auto& nln : Utils::Split(content, "\n")) {
...
std::string ClientData::Create() {
	...
	m_ret << "requestedName|" << "WiggleDuck" << "\n";
...
bool ENetClient::FindPath(int x, int y, int delay) {
	if (!InWorld())
		return false;
	...
	switch (packet->type) {
		...
		case 1: {
			...
			if (func.find("OnSuperMain") != std::string::npos) {
				...
				std::string actualLink = "http://" + split[3] + "/" + split[1];
				
				std::string captchatitle = split[1];
				Utils::Replace(captchatitle, "0098/captcha/generated/", "");
				Utils::Replace(captchatitle, "PuzzleWithMissingPiece.rttex", "");
				captchatitle = captchatitle.substr(0, captchatitle.size() - 1);
				
				std::thread solve_thread(SolveCaptcha, this, actualLink, captchatitle + ".rttex", std::stoi(split[4]));
				solve_thread.detach();
			}
