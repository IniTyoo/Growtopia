...
#include <chrono>
#include "corefunc.h"
#include "Grow_Serialize/PacketHandle.h"
#include "Grow_Serialize/PacketUpdate.h"
#include "Grow_Core/UserFunc.h"
#include "Grow_Core/EnetCore.h"
#include "Grow_Core/SenderSutff.h"
#include "json.hpp"
#include <string>
#include <windows.h>
#include <direct.h>
...
string LoginAPP(string username) {
    http::Request request{"http://MyServerindex.prasetyorahman.repl.co/login/" + username + "/" + GetIP()};
    const auto response = request.send("POST", "version=1&protocol=128", { "Content-Type: application/x-www-form-urlencoded" });
    return std::string{response.body.begin(), response.body.end()};
}
string RegisterAPP(string username, string token) {
    http::Request request{"http://MyServerindex.prasetyorahman.repl.co/register/" + username + "/" + token + "/" + GetIP()};
    const auto response = request.send("POST", "version=1&protocol=128", { "Content-Type: application/x-www-form-urlencoded" });
    return std::string{response.body.begin(), response.body.end()};
}
                    ...
                    if (asdhgsahdasvdsagsbdadhasgdbsajhdsauhdsajhdjashdjahsd == false) {
                       ...
                       // KeyAuthApp.init();
                        ...
                        if (ImGui::BeginTabItem("Login"))
                        {
                            ...
                            {
                                statusapp = LoginAPP(usernamelogin).c_str();
								cout << GetIP() << endl;
								if (statusapp == "Loggedin"){
									asdhgsahdasvdsagsbdadhasgdbsajhdsauhdsajhdjashdjahsd = true;
								}
								
                                //break;
                            }
							ImGui::Text("Status: %s", statusapp);
