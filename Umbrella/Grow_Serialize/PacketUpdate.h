...
void GrowtopiaBot::OnCallFunction(BYTE* structPointer, ENetPacket* packets){
	try{
...
void GrowtopiaBot::OnTileActivateRequest(BYTE* structPointer, ENetPacket* packets){
	PlayerMoving* datak = unpackPlayerMoving(structPointer);
	cout << "Type 7: " << datak->netID << endl;
}

void GrowtopiaBot::OnSendItemDatabaseData(BYTE* structPointer, ENetPacket* packets){
...
void GrowtopiaBot::OnModifyItemInventory(BYTE* structPointer, ENetPacket* packets){
	auto packet = get_struct(packets);
...
void GrowtopiaBot::OnPingRequest(BYTE* structPointer, ENetPacket* packets){
	PlayerMoving* datak = unpackPlayerMoving(structPointer);
	datak->packetType = 21;
...
void GrowtopiaBot::OnSendTileTreeState(BYTE* structPointer, ENetPacket* packets){
	string x = std::to_string(*(int*)(structPointer + 44));
	string y = std::to_string(*(int*)(structPointer + 48));
}

void GrowtopiaBot::OnItemChangeObject(BYTE* structPointer, ENetPacket* packets){
	auto packet = get_struct(packets);
...
void GrowtopiaBot::OnTileChangeRequest(BYTE* structPointer, ENetPacket* packets){
	auto packet = get_struct(packets);
...
void GrowtopiaBot::OnSendInventoryState(BYTE* structPointer, ENetPacket* packets){
	inventory.clear();
	auto packet = packets;
...
void GrowtopiaBot::OnSendMapData(BYTE* structPointer, ENetPacket* packets){
	tiles.clear();
			floatItem.clear();
			last_oid = 0;
			auto packet = packets;
